...
**My fork is `kiaragrasso/ai_agent_workshop`.**

Anything that **creates** — `gh issue create`, `gh pr create` — passes
`--repo kiaragrasso/ai_agent_workshop`. Never write to
...
**`mytools` is written in R.** Every subcommand and every test. Don't introduce a
second language without asking me.
